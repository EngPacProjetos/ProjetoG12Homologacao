 ...
 * @CRIACAO 06/07/2026
 * @PURPOSE CRIA UM DATASET RESPONSAVEL POR EXTRAIR TODAS AS INFORMACOES DE HISTORICO E STATUS DE NOTAS FISCAIS (ENVIO E AUTORIZAÇÃO)
 * @CONSULTA G12HISTORICONFS
 */
function createDataset(fields, constraints, sortFields) {
    ...
    // Colunas alinhadas com o retorno real da consulta G12HISTORICONFS
    var COLUNAS = [
        "HISTORICO",
        "STATUS"
    ];
    ...
    try {
        ...
        log.info("[G12-HISTORICO-NFSE] Usuario de execucao: " + usuario);
        ...
        if (IDMOV == undefined || IDMOV == null || String(IDMOV).trim() == "") {
            log.error("[G12-HISTORICO-NFSE] IDMOV nao foi informado. Abortando.");
        ...
        if (CODCOLIGADA == undefined || CODCOLIGADA == null || String(CODCOLIGADA).trim() == "") {
            log.error("[G12-HISTORICO-NFSE] CODCOLIGADA nao foi informado. Abortando.");
        ...
        log.info("[G12-HISTORICO-NFSE] PARAMS enviados: " + PARAMS);

        var result = authService.realizarConsultaSQL("G12HISTORICONFS", 0, "F", PARAMS);
        log.info("[G12-HISTORICO-NFSE] Retorno bruto do RM: " + result);

        var JSONObj = org.json.XML.toJSONObject(result);
        log.info("[G12-HISTORICO-NFSE] JSON parseado: " + JSONObj);

        var dados = JSONObj.get("NewDataSet").get("Resultado");
        log.info("[G12-HISTORICO-NFSE] Dados extraidos: " + dados);

        // Funcao auxiliar para montar uma linha com fallback seguro
        function buildRow(row) {
            return new Array(
                row.has("HISTORICO") ? row.get("HISTORICO") : "",
                row.has("STATUS") ? row.get("STATUS") : ""
            );
        }

        if (dados.isNull(0)) {
            log.info("[G12-HISTORICO-NFSE] Registro unico encontrado.");
            dataset.addRow(buildRow(dados));

        } else {
            // Multiplos registros
            log.info("[G12-HISTORICO-NFSE] Multiplos registros encontrados: " + dados.length());
        ...
        log.error("[G12-HISTORICO-NFSE] ERRO: " + String(e) + " | Linha: " + e.lineNumber);
