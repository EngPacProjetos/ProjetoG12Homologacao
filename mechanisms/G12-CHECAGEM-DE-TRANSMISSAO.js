...
function getGroup(CentroDeCusto, CodColigada, Filial) {
	...
	if (CodColigada == 2) {
		if (CentroDeCusto == "02.01.01.01.001" /* ADM REGIONAL JOAO PESSOA */
			...
			userList.add('Pool:Group:G12-CHECAGEMTRANSMISSAO-CONTRATOS-PB');
		} else {
			userList.add('Pool:Group:G12-CHECAGEMTRANSMISSAO-CONTRATOS-GERAL');
		}
	} else if (CodColigada == 1) {
		if (Filial == 1 || Filial == 5) {
			userList.add('Pool:Group:G12-CHECAGEMTRANSMISSAO-CONTRATOS-DFGO');
		} else {
			userList.add('Pool:Group:G12-CHECAGEMTRANSMISSAO-CONTRATOS-GERAL');
		}
